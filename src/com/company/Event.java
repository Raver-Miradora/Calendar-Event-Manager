...
public class Event implements Comparable {
    ...
    public int compareTo(Object o) {
        ...
        // Serialize Event attributes into a string representation
        stringBuilder.append(name).append(",");
        stringBuilder.append(timeInterval.getStartTime()).append(",");
        stringBuilder.append(timeInterval.getEndTime()).append(",");
        stringBuilder.append(repeatedDays).append(",");
        for (LocalDate date : dates) {
            stringBuilder.append(date.format(DateTimeFormatter.ofPattern("MM/dd/yyyy"))).append(",");
        }
        return stringBuilder.toString();
    }

    public static Event parseFromFileString(String eventData) {
        String[] parts = eventData.split(",");
        String name = parts[0];
        String startTime = parts[1];
        String endTime = parts[2];
        String repeatedDays = parts[3];
        TimeInterval timeInterval = new TimeInterval(startTime, endTime);
        TreeSet<LocalDate> dates = new TreeSet<>();
        for (int i = 4; i < parts.length; i++) {
            LocalDate date = LocalDate.parse(parts[i], DateTimeFormatter.ofPattern("MM/dd/yyyy"));
            dates.add(date);
        }
        Event event = new Event(name, repeatedDays, startTime, endTime, 
                                dates.first().toString(), dates.last().toString());
        event.dates.addAll(dates);
        return event;
    }
